...
positions connected by thin lines. All points use the same color.

Usage:
    python configurations/generate.py
"""
...
def compute_scale_factor(n_robots: int, base_n: int = BASE_N) -> float:
    """Mirror sweep scaling: scale with sqrt(N / base_N)."""
    return math.sqrt(n_robots / base_n)


def make_config(n_robots: int) -> Config:
    scale = compute_scale_factor(n_robots)
    time_horizon = BASE_TIME * scale
    cluster_radius = BASE_CLUSTER_RADIUS * scale
    timestep = time_horizon / TIMESTEPS_K
...
def plot_scenario(ax, n_robots: int, seed: int):
    ...
    cluster_radius = cfg.problem.scenario.cluster_radius
    ...
    # Info text
    info = (
        f"$N={n_robots}$, seed={seed},  $R={MIN_DISTANCE}$\\,m,  "
        f"$r_{{\\mathrm{{cluster}}}}={cluster_radius:.2f}$\\,m"
    )
